...
{
    public sealed class ProjectManager : ModuleBase
    {
        private static readonly LocalSystemUser user = new LocalSystemUser(new Guid("{2CA93C06-7C49-4872-81DA-3D280F2FCAE6}"), NameReflectionUtils.GetName(typeof(ProjectManager)), UserState.Active);
        private LogProviderBase logger;
        private ProjectProviderFactoryBase factory;

        public ProjectProviderBase GetProvider()
        {
            if (this.State == RunState.Running)
                return this.factory.GetProvider(new UserSecurityContext(null));
            return null;
        }
        public ProjectProviderBase GetProvider(UserSecurityContext context)
        {
            if (this.State == RunState.Running)
        ...
        public ProjectStatusTypeProviderBase GetStatusTypeProvider()
        {
            if (this.State == RunState.Running)
                return this.factory.GetStatusTypeProvider(new UserSecurityContext(null));
            return null;
        }
        public ProjectStatusTypeProviderBase GetStatusTypeProvider(UserSecurityContext context)
        {
            if (this.State == RunState.Running)
                return this.factory.GetStatusTypeProvider(context);
            return null;
        }
        ...
        protected override void InitializeImpl()
        {
            lock (instance)
            {
                if (RuntimeUtils.Initializable(this.State))
                {
                    ...
                    this.logger = LogManager.Instance.GetProvider(typeof(ProjectManager));
                    Log(meth, LogLevel.Info, "Called");

                    ConfigurationProviderBase config = ConfigurationManager.Instance.GetProvider();
                    if (config != null)
                    {
                        ConfigurationParameter param = config.Get(typeof(ProjectManager), "provider");
                        if (param != null)
                        {
        ...
        public void Initialize(TypeNameReference providerFactory)
        {
            lock (instance)
            {
                if (RuntimeUtils.Initializable(this.State) || this.State == RunState.Initializing)
                {
                    ...
                    {
                        this.factory = NameReflectionUtils.CreateInstance<ProjectProviderFactoryBase>(providerFactory);
